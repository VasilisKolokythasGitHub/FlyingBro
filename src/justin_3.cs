...
public class justin_3 : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D collider) {
		...
		if(collider.tag == "Player" && !destroyed3  && efage_laser) { // an pesei panw kai dn exei katastrafei to nokia
			destroyed3 = true;						//katastrafhke
		...
		if(collider.tag == "Player" && !destroyed3  && efage_bounidi) { // an pesei panw kai dn exei katastrafei to nokia
			destroyed3 = true;						//katastrafhke
		...
		if(collider.tag == "UnPunch" && !destroyed3 ){     
